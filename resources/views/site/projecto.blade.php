    ...
    <div class="banner banner-static">
        <div class="container">
            <div class="content row">
                ...
                <div class="imagebg">
                    <img src="{{ asset(Voyager::image(setting('site.projecto_img_servico'))) }}" alt="">
                </div>
                    ...
                    <ul class="photos-list col-x3">
                        @forelse (App\Models\Projecto::all() as $projecto)
                            <li class="filtr-item" data-category="{{ $projecto->projecto_categoria_id }}">
                                <div class="photo">
                                    <img src="@if ($projecto->imagens == null) {{ asset('image/gallery/gallery1.jpg') }}
                                    @else
                                        {{ asset(Voyager::image(json_decode($projecto->imagens)[0])) }} @endif"
                                        alt="{{ $projecto->titulo }}">
                                    <div class="photo-item-link">
                                        <span class="link-block">
                                            ...
                                            <a class="link link-popup single-lightbox"
                                                href="@if ($projecto->imagens == null) {{ asset('image/gallery/gallery1.jpg') }}
                                    @else
                                        {{ asset(Voyager::image(json_decode($projecto->imagens)[0])) }} @endif"
                                                title="{{ $projecto->titulo }}"><em class="fa fa-arrows-alt"></em></a>
                                        </span>
                                    </div>
